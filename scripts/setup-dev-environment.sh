...
echo -e "${BLUE}╔═══════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   QUAD Framework - Developer Environment Setup   ║${NC}"
echo -e "${BLUE}╚═══════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ -z "$BW_SESSION" ]; then
    echo -e "${YELLOW}⚠️  BW_SESSION not set${NC}"
    echo -e "${BLUE}Run: export BW_SESSION=\$(bw unlock --raw)${NC}"
    exit 1
fi

echo -e "${GREEN}✅ Vaultwarden connection verified${NC}"
echo ""
...
echo -e "${BLUE}📥 Fetching credentials from Vaultwarden...${NC}"
echo ""
...
echo -e "${GREEN}✅ quad-web/.env.local created${NC}"
echo ""
...
if ! grep -q ".env.local" quad-web/.gitignore 2>/dev/null; then
    ...
    echo -e "${YELLOW}⚠️  Added .env.local to quad-web/.gitignore${NC}"
fi

if ! grep -q ".env.deploy" .gitignore 2>/dev/null; then
    echo ".env.deploy" >> .gitignore
    echo -e "${YELLOW}⚠️  Added .env.deploy to .gitignore${NC}"
fi
...
echo -e "  ✅ quad-web/.env.local (for local development)"
echo -e "  ✅ .env.deploy (for deployment scripts)"
echo ""
